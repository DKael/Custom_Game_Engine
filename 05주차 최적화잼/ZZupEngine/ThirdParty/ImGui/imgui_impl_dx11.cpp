...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the pixel shader
    {
        static const char* pixelShader =
            ...
            float3 useLow = step(color, float3(0.04045, 0.04045, 0.04045));\
            return lerp(high, low, useLow);\
            }\
            \
            float4 main(PS_INPUT input) : SV_Target\
            {\
            float4 tint = float4(SrgbToLinear(input.col.rgb), input.col.a); \
            float4 out_col = tint * texture0.Sample(sampler0, input.uv); \
            return out_col; \
            }";
