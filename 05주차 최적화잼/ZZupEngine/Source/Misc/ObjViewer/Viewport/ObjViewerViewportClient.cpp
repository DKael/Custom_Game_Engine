...
void FObjViewerViewportClient::CreateCamera()
{
    ...
    const FVector InitialLocation =
        Settings ? Settings->InitViewPos : FVector(10.0f, 0.0f, 5.0f);
    const FVector InitialLookAt =
        Settings ? Settings->InitLookAt : FVector(0.0f, 0.0f, 0.0f);

    Camera->SetLocation(InitialLocation);
    Camera->SetLookAt(InitialLookAt);

    const uint32 SafeWidth = static_cast<uint32>(WindowWidth > 0.0f ? WindowWidth : 1.0f);
    const uint32 SafeHeight = static_cast<uint32>(WindowHeight > 0.0f ? WindowHeight : 1.0f);
    Camera->OnResize(SafeWidth, SafeHeight);
...
void FObjViewerViewportClient::HandleDragStart(const FRay& Ray)
{
    ...
    FObjViewerRenderPipeline* RenderPipeline = Engine ? Engine->GetRenderPipeline() : nullptr;
    if (RenderPipeline)
    {
        TArray<int32> CandidateIndices;
        TArray<float> CandidateTs;
        RenderPipeline->QueryRayCandidates(Ray, CandidateIndices, CandidateTs);

        for (int32 i = 0; i < static_cast<int32>(CandidateIndices.size()); ++i)
        {
            if (CandidateTs[i] > ClosestDistance)
            {
                break;
            }

            HitResult = {};
            if (RenderPipeline->RaycastCandidateComponent(CandidateIndices[i], Ray, HitResult))
            {
                if (HitResult.Distance < ClosestDistance)
                {
                    ClosestDistance = HitResult.Distance;
                    BestActor = HitResult.HitComponent ? HitResult.HitComponent->GetOwner()
                                                       : nullptr;
                }
            }
        }
	...
	LastPickTime = FWindowsPlatformTime::ToMilliseconds(PickCounter.Finish());
