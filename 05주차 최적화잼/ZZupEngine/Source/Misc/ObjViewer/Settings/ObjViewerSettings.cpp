...
namespace ObjViewerKey
{
	...
	constexpr const char* CameraForwardSensitivity = "CameraForwardSensitivity";
	...
	constexpr const char* bOcclusionCulling = "bOcclusionCulling";
...
void FObjViewerSettings::SaveToFile(const FString& Path) const
{
	...
	Viewport[ObjViewerKey::CameraForwardSpeed] = CameraForwardSpeed;          // Dolly Speed
	Viewport[ObjViewerKey::CameraMoveSensitivity] = CameraMoveSensitivity;      // Panning Speed
	Viewport[ObjViewerKey::CameraRotateSensitivity] = CameraRotateSensitivity;  // Rotation Speed
	...
	ViewObj[ObjViewerKey::bOcclusionCulling] = bOcclusionCulling;
...
void FObjViewerSettings::LoadFromFile(const FString& Path)
{
	...
	if (Root.hasKey(ObjViewerKey::Viewport))
	{
		...
		if (Viewport.hasKey(ObjViewerKey::CameraForwardSpeed))
			CameraForwardSpeed = static_cast<float>(Viewport[ObjViewerKey::CameraForwardSpeed].ToFloat()); // Dolly Speed
		if (Viewport.hasKey(ObjViewerKey::CameraMoveSensitivity))
			CameraMoveSensitivity = static_cast<float>(Viewport[ObjViewerKey::CameraMoveSensitivity].ToFloat()); // Panning Speed
		if (Viewport.hasKey(ObjViewerKey::CameraRotateSensitivity))
			CameraRotateSensitivity = static_cast<float>(Viewport[ObjViewerKey::CameraRotateSensitivity].ToFloat()); // Rotation Speed
	...
	if (Root.hasKey(ObjViewerKey::View))
	{
		...
		if (ViewObj.hasKey(ObjViewerKey::bOcclusionCulling))
			bOcclusionCulling = ViewObj[ObjViewerKey::bOcclusionCulling].ToBool();
	}
