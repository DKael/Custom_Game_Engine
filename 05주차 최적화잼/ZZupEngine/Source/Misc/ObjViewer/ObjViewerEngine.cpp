...
void UObjViewerEngine::Init(FWindowsWindow* InWindow)
{
    ...
    {
        auto Pipeline = std::make_unique<FObjViewerRenderPipeline>(this, Renderer);
        RenderPipelinePtr = Pipeline.get();
        SetRenderPipeline(std::move(Pipeline));
    }

    // 초기 실행 시 WM_SIZE를 놓친 경우가 있어, 렌더러/뷰포트 상태를 현재 창 크기로 즉시 동기화한다.
    const uint32 InitialWidth = InWindow->GetWidth() > 0.0f
                                    ? static_cast<uint32>(InWindow->GetWidth())
                                    : 1u;
    const uint32 InitialHeight = InWindow->GetHeight() > 0.0f
                                     ? static_cast<uint32>(InWindow->GetHeight())
                                     : 1u;
    OnWindowResized(InitialWidth, InitialHeight);
}
...
void UObjViewerEngine::OnWindowResized(uint32 Width, uint32 Height)
{
    ...
    ViewportClient.SetViewportRect(0.0f, 0.0f, SafeWidth, SafeHeight);
}
