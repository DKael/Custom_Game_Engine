                                    ...
                                    ID3D11DeviceContext*          Context)
{
    ...
    // RTV 없이 Depth Prepass DSV(쓰기)만 바인딩
    ...
    // VS 바인드 (PixelShader == null → PSSetShader(null) 자동 호출)
    Resources.DepthPrepassShader.Bind(Context);

    // b1: PerObjectConstants (Model matrix) 바인딩
...
void FRenderer::ExecuteHiZDownsample(ID3D11DeviceContext* Context)
{
    ...
    // CS 디스패치 전 명시적 clean state 보장.
    // Depth prepass depth를 Hi-Z로 복사하기 전에 Render()에서 이미 OM 언바인드를 수행한다.
    ...
    // mip 1부터 시작 (mip0은 Step 4에서 CopySubresourceRegion으로 채움)
                                     ...
                                     const TArray<FAABB>&  AABBBoundsArray)
{
    ...
    // 1. AABB 데이터 업로드 (DYNAMIC Map/Unmap)
    ...
    // 5. 결과를 staging 슬롯으로 복사하고 비동기 완료 query를 건다.
                                              ...
                                              bool                 bApplyHideHysteresis)
{
    ...
    for (uint32 i = 0; i < static_cast<uint32>(SubmittedIndices.size()); ++i)
    {
        ...
        uint8& ConsecutiveOccludedCount =
            OcclusionConsecutiveOccludedCounts[RenderCommandIndex];
