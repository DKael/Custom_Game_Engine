...
// StaticMeshBuffer (b6) — ShaderStaticMesh
// 
// 
// 대응
...
// AABB GPU 업로드 구조체 (StructuredBuffer element, 32 bytes)
...
// OcclusionCull CS cbuffer b0
struct FOcclusionCullConstants
{
...
// 16-bytes aligned
struct FPerformanceStaticMeshConstants
{
    FVector DiffuseColor;   // 12 bytes
    float   bHasDiffuseMap; // 4 bytes (0.0 or 1.0)

	// 직접 GPU에 전송하지 않는 데이터 (CPU-only)
    ID3D11ShaderResourceView* DiffuseSRV = nullptr;
};

// 16-bytes aligned
struct FPerObjectPerformanceConstants
{
...
struct FRenderCommand
{
	...
	FPerObjectPerformanceConstants PerObjectPerformanceConstants = {};
	...
	union
	{
		...
		FPerformanceStaticMeshConstants PerformanceStaticMesh;
	} Constants;
