...
{
	// 현재 컴포넌트에 대한 Visible 체크가 필요하지 않으므로 불필요한 IsVisible 반환 삭제 
	...
	case EPrimitiveType::EPT_StaticMesh:
	{
		...
		FMeshBuffer* MeshBuffer = MeshBufferManager.GetPerformanceMeshBuffer(StaticMesh);
		...
		for (int32 SectionIdx = 0; SectionIdx < static_cast<int32>(Sections.size()); ++SectionIdx)
		{
			...
			Cmd.Constants.PerformanceStaticMesh.DiffuseColor = MtlData->DiffuseColor;

			ID3D11ShaderResourceView* DefaultSRV = FResourceManager::Get().GetDefaultWhiteSRV();

			if (ViewMode == EViewMode::Wireframe)
			{
				Cmd.Constants.PerformanceStaticMesh.bHasDiffuseMap = 1.0f;
				Cmd.Constants.PerformanceStaticMesh.DiffuseSRV = DefaultSRV;
			}
			else
			{
				// LoadMaterial 시점에 캐싱된 SRV 포인터를 직접 참조.
				// 기존 ResolveSRV()의 매 프레임 unordered_map<FString> 조회(문자열 해싱) 제거.
				ID3D11ShaderResourceView* DiffuseSRV = (MtlData->bHasDiffuseTexture && MtlData->CachedDiffuseSRV)
					? MtlData->CachedDiffuseSRV : DefaultSRV;
				Cmd.Constants.PerformanceStaticMesh.bHasDiffuseMap = MtlData->bHasDiffuseTexture ? 1.0f : 0.0f;
				Cmd.Constants.PerformanceStaticMesh.DiffuseSRV = DiffuseSRV;
			}
