...
// StaticMesh WorldMatrix 배열 등을 GPU에 한 번에 전달하는 StructuredBuffer
class FStructuredBuffer
{
public:
	void Create(ID3D11Device* Device, uint32 ElementCount, uint32 ElementStride);
	void Update(ID3D11DeviceContext* Context, const void* Data, uint32 ByteSize);
	void Release();
...
public:
	...
	void CreateForPerformanceMesh(ID3D11Device* InDevice, const TArray<FPerformanceVertex>& InVertices, const TArray<uint32>& InIndices);
	void Release();
