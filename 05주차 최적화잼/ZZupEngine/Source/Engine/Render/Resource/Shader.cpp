		...
		const D3D11_INPUT_ELEMENT_DESC * InInputElements, UINT InInputElementCount)
{
	...
	// Pixel Shader 컴파일 및 생성 (PSEntryPoint가 없으면 스킵 — depth-only pass 지원)
...
{
	TComPtr<ID3DBlob> csCSO;
	TComPtr<ID3DBlob> errorBlob;

	HRESULT hr = D3DCompileFromFile(InFilePath, nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE,
		InCSEntryPoint, "cs_5_0", 0, 0, csCSO.GetAddressOf(), errorBlob.GetAddressOf());
	if (FAILED(hr))
	{
		if (errorBlob)
	...
	hr = InDevice->CreateComputeShader(csCSO->GetBufferPointer(), csCSO->GetBufferSize(), nullptr,
