...
bool FResourceManager::LoadMaterial(const FString& MtlFilePath)
{
	...
	{
		if (Mat.bHasDiffuseTexture && !Mat.DiffuseTexPath.empty())
		{
			LoadTexture(Mat.DiffuseTexPath, CahcedDevice);
			// 렌더 루프에서 매 프레임 문자열 조회를 피하기 위해 SRV 포인터를 직접 캐싱.
			// LoadTexture가 이미 캐시에 올린 결과를 한 번만 참조해 저장한다.
			if (!Mat.CachedDiffuseSRV)
			{
				FMaterialResource* Res = FindTexture(Mat.DiffuseTexPath);
				Mat.CachedDiffuseSRV = (Res && Res->SRV) ? Res->SRV : nullptr;
			}
		}
...
// Texture Cache Miss를 해결하기 위해 Texture MipMap을 자동 생성하도록 변경한다.
FMaterialResource* FResourceManager::LoadTexture(const FString& Path, ID3D11Device* Device)
{
	...
	// 밉맵 자동 생성을 위해 ImmediateContext를 가져옴
	...
	{
		// DDS: 파일 내 밉맵 데이터 로드, 없으면 Context로 자동 생성
		hr = DirectX::CreateDDSTextureFromFile(Device, Context, FullPath.c_str(), nullptr, &Resource.SRV);
	}
	else
	{
		// WIC (png/jpg/jpeg): Context 전달 시 GenerateMips 자동 호출
		hr = DirectX::CreateWICTextureFromFile(Device, Context, FullPath.c_str(), nullptr, &Resource.SRV);
	}
