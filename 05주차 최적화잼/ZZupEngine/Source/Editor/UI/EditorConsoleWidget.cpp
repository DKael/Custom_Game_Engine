...
FEditorConsoleWidget::FEditorConsoleWidget() 
{
    ...
    // 엔진 코어 로거(FLogger)에 자신을 구독시킴
    // FLogger에서 로그가 발생하면, 이를 받아와 AddLog를 호출하여 UI에 띄운다.
    FLogger::AddSubscriber([](const std::string& LogMessage) {
        FEditorConsoleWidget::AddLog("%s", LogMessage.c_str());
    });
}
